...
async function getProblemFromLeetCode(slug){
  const res = await fetch("https://leetcode.com/graphql",{
    ...
    body: JSON.stringify({
      query:`query($s:String!){question(titleSlug:$s){title questionFrontendId}}`,
      variables:{ s: slug }
    })
  });
  ...
  while(keyIndex < API_KEYS.length){
    try{
      const res = await fetch(
        `https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&maxResults=10&q=${q}&channelId=${channelId}&key=${getKey()}`
      );
...
function showVideo(videoId, title, mode){
  ...
  if(mode==="video"){
    ...
    content = `
    <div style="height:160px;display:flex;flex-direction:column;align-items:center;justify-content:center;color:#aaa;text-align:center;">
      <div style="font-size:22px;">📹</div>
      <div>No high-quality solution available</div>
      <div style="font-size:12px;opacity:0.7;margin-top:4px;">This may be a new or recent contest problem,</div>
      <div style="font-size:12px;opacity:0.7;">or an older problem without a proper solution video.</div>
      <div style="font-size:12px;opacity:0.7;margin-top:6px;">The video will appear once a good solution is found.</div>
    </div>`;
  }else{
  ...
  panel.querySelector("#lc-close").onclick=()=>{
    ...
    if(hasVideo){const f=document.getElementById("lc-frame");f.src=f.src;}
  };
...
async function main(){
  ...
  const {questionFrontendId:lcNo,title}=await getProblemFromLeetCode(slug);
  ...
  let fallback=null, quotaDead=false;

  for(const ch of CHANNELS){
    const yt=await searchLC(lcNo,ch);
    if(yt?.quotaExhausted){quotaDead=true;break;}
  ...
  if(fallback){
    cache[lcNo]={videoId:fallback.id.videoId,title:fallback.snippet.title,mode:"video"};
    setCache(cache);
